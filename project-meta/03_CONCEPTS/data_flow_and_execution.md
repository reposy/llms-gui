...
### 입력 매핑과 실행 책임의 단일화 (2024.06 이후)

- InputRow[] → value[] 변환(입력 매핑)은 오직 flow 실행 서비스(executeFlowExecutor)에서만 수행합니다.
- 수동 실행, 체인 실행, 자동화 등 모든 실행 경로에서 flow.inputs를 변환 없이 그대로 전달합니다.
- 실행 서비스는 입력이 InputRow[] 타입이면 자동으로 value[]로 변환하여 실행합니다.
- 이 구조는 단일 진입점/단일 책임 원칙을 지키며, 중복 변환/이중 변환/누락 문제를 방지합니다.
- 불필요한 디버그/상세 로그는 제거되어, 프로덕션 환경에서도 깔끔하게 동작합니다.

### 파일 데이터 처리 (2024.12 추가)

애플리케이션에서 파일은 통합 파일 관리 시스템을 통해 처리됩니다:

*   **파일 타입**: `File` 객체(로컬), `LocalFileMetadata`(레거시), `BackendFileMetadata`(신규 통합 시스템)
*   **컨텍스트 기반 저장**: 파일은 용도에 따라 분류되어 저장됩니다 (flow_executor, flow_editor, input_node)
*   **영속성**: 백엔드 저장으로 새로고침 시에도 파일이 유지됩니다
*   **메타데이터 전달**: 파일 실행 시 `BackendFileMetadata`가 노드 간 전달되어 파일 정보를 보존합니다 