...
`Input` 노드는 워크플로우의 시작점을 정의하거나, 다른 노드로부터 받은 데이터를 저장하고 후속 처리를 위한 실행 방식을 제어하는 노드입니다. 사용자가 직접 텍스트나 파일을 입력할 수 있으며, 여러 개의 입력 항목(items)을 관리합니다.

-   **입력:** 다른 노드로부터 데이터를 받을 수 있습니다. 받은 데이터는 기존 항목 목록(`items`)에 추가됩니다. (빈 객체는 추가되지 않음)
-   **처리 (`core/InputNode.ts`):**
    -   주된 역할은 데이터를 저장하고, 설정된 실행 모드(`executionMode` 또는 `iterateEachRow` 속성)에 따라 후속 노드로 데이터를 전달하는 것입니다.
    -   실행 시, 이전 노드로부터 입력(`input`)이 있으면 기존 `items` 배열에 추가합니다.
    -   `iterateEachRow` 설정값 (UI에서는 Execution Mode 토글 - Batch/For Each)을 확인합니다.
-   **출력:** 실행 모드에 따라 달라집니다.
    -   **Batch 모드 (`iterateEachRow: false`):** 저장된 모든 항목(`items`)을 포함하는 **배열 전체**를 다음 노드로 전달합니다.
    -   **For Each 모드 (`iterateEachRow: true`):** 각 항목(`item`)에 대해 개별적으로 후속 노드 실행을 트리거하고, Input 노드 자체는 다음 노드로 데이터를 전달하지 않습니다 (`null` 반환). 즉, 체이닝이 Input 노드에서 멈추고 각 항목에 대한 분기 실행이 시작됩니다.

### 2. 프론트엔드 UI (노드 설정 패널 - `InputNodeConfig.tsx`)

-   **Processing Mode (항목이 2개 이상일 때 표시):**
    -   `Batch`: 모든 항목을 하나의 배열로 묶어 다음 노드에 전달합니다. 후속 노드는 전체 배열을 입력으로 받습니다.
    -   `For Each`: 각 항목을 개별적으로 다음 노드에 전달합니다. 후속 노드는 각 항목에 대해 한 번씩, 총 여러 번 실행됩니다. (그룹 노드와 연동하여 반복 실행에 사용됨)
-   **Add Text:** 텍스트 영역에 내용을 입력하고 'Add Text' 버튼을 눌러 텍스트 항목을 `items` 목록에 추가합니다.
-   **Add Files:** 'Choose Files' 버튼을 통해 로컬 파일을 업로드하여 파일 항목(`File` 객체)을 `items` 목록에 추가합니다.
-   **Input Items:** 현재 노드에 저장된 항목 목록을 보여줍니다. (텍스트 미리보기 또는 파일 이름)
    -   각 항목 옆의 휴지통 아이콘으로 개별 항목을 삭제할 수 있습니다.
    -   'Clear All' 버튼으로 모든 항목을 삭제할 수 있습니다.
-   **(UI 하단 요약)** 현재 저장된 항목 수와 선택된 실행 모드가 표시됩니다.
